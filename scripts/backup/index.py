...
def gbrowse(req):
	return _page_result(req, 'gbrowse')
...
def blast_app(req):
    """calls scripts/blast.py function
    """
...
def dotplot_app(req, block=0, chr1=None, chr2=None, ks=0, ks_low=0, ks_high=99, sp=0, species1='os', species2='os'):
    """calls scripts/plot.py function
    """
    dotplot = import_module('dotplot')
    s = dotplot.dotplot1(req,block,chr1,chr2,ks,ks_low,ks_high,sp,species1,species2)
    vars = {"menu": _menu(req, hlight='dotplot'),
            "body": psp.PSP(req, _tmpl_path('dotplot_body.htm'), vars={'output':"<div class='monotext'>"+s+"</div>"})}
    return psp.PSP(req, _tmpl_path(_MAIN_TMPL), vars=vars)

def locus_app(req):
    """calls scripts/mysql.py function
    """
