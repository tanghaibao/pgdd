...
def locus1(req):
    ...
    if len(results)==0:
        ...
        else: data = "<font color='red'>The locus ( %s ) is not contained within any block.</font><br /><hr />%s"%(get_details(lc),lc_details)
        return data
    else:
...
def show_seg(seg_id,lc,note,si,sc):
    ...
    genelt_a = myconnect(sql5)
    genelt_b = myconnect(sql6)
...
def segplot1(genelt_a,genelt_b,hit_col,block_results,lc,species,chr_a,chr_b,start_a,stop_a,start_b,stop_b):
    ...
    # species names
    root.text(sp+bw*3,mp+.15,sp_dict[species_a.lower()],verticalalignment="center",bbox=dict(fc='k',alpha=.5,pad=5,lw=0),color="w")
    root.text(sp+bw*3,mt-.15,sp_dict[species_b.lower()],verticalalignment="center",bbox=dict(fc='k',alpha=.5,pad=5,lw=0),color="w")
    # physical position
    ...
    for a in genelt_a:
        ...
        for datum in block_results:
            if a[0]==datum[hit_col] and datum[other_col] in genedt_b:
	        ...
	        root.text(center_a,mp+.06,a[0].split("g")[1],rotation=45,color=cc,size=7,horizontalalignment='center')
	        root.text(center_b,mt-.18,datum[other_col].split("g")[1],rotation=45,color=cc,size=7,horizontalalignment='center')
